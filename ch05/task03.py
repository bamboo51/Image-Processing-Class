...
def low_pass_filter(M, N, cutoff=10):
    """
    Creates a low-pass filter of size MxN with a cutoff frequency.
    """
    ...
    for y in range(M):
        for x in range(N):
            ...
            H[y, x] = 1.0 if distance <= cutoff else 0.0

    return H


def high_pass_filter(M, N, cutoff=10):
    """
    Creates a high-pass filter of size MxN with a cutoff frequency.
    """
    H = np.zeros((M, N), dtype=np.float64)
    ...
    for y in range(M):
        for x in range(N):
            distance = np.sqrt((y - cy) ** 2 + (x - cx) ** 2)
            H[y, x] = 0.0 if distance <= cutoff else 1.0
...
if __name__ == "__main__":
    ...
    for idx, (p, q) in enumerate(zip(P, Q)):
        ...
        # Low-pass filter
        H = low_pass_filter(M, N, cutoff=2)

        F_filtered_shifted = fftshift2d(F) * H
    ...
    for idx, (p, q) in enumerate(zip(P, Q)):
        ...
        # Low-pass filter
        H = high_pass_filter(M, N, cutoff=2)
